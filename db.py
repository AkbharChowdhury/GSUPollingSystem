...
class Database:
    ...
    def get_candidate_by_position(self, position):
        ...
        if position == Helper.all_positions():
            return self.__cur.execute("""
                    SELECT
                    c.`firstname`,
                    c.`lastname`,
    ...
    def generate_list(self):
        position_list = self.get_candidate_positions()
        de = deque([item for positions in position_list for item in positions])
        de.appendleft(Helper.all_positions())
        return list(de)
