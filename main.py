...
class Navigation:
    ...
    """This class is used for navigation purposes using file menu as the primary navigation method
    ...
    def file_menu(self):
        ...
        module_menu = Menu(menu, tearoff=False)
        ...
        menu.add_cascade(menu=module_menu, label="File")

        # Create a pull-down menu for help operations
        profile_menu = Menu(menu, tearoff=False)
        ...
        menu.add_cascade(menu=profile_menu, label="Profile")
        self.master.config(menu=menu)


class CheckLogin(Navigation):
    def is_logged_in(self, message: str = 'This page is restricted, you must be logged in'):
        if not db.student_id:
            tm.showerror('Login Error', message)
...
class Login(Navigation):
    def authenticate(self):
        ...
        if Validation.is_blank(username, password.strip()):
            tm.showerror('Login Error', 'Username and password is required!')
            return

        if not db.username_exists(username):
            tm.showerror('Invalid username', 'This username does not exist!\nPlease try again.')
            return

        if db.is_authorized(username, password):
    ...
    def __init__(self, master):
        self.master = master
        master.title("Student Login Form")
...
class Candidate(Navigation):
    ...
    def search_by_candidate_position(self):
        ...
        # position widget
        # label
        self.label_position = Label(self.master, text='search by candidate position'.capitalize(), font=('bold', 14))
        self.label_position.grid(row=1, column=2, sticky=W)
        # combo
        self.combo_search_position = Combobox(self.master, state='readonly', width=20)
        self.combo_search_position['values'] = db.generate_list()
...
class Vote(Navigation):
    ...
    def __init__(self, master):
        ...
        # Populate initial list
        self.populate_list()
        CheckLogin.is_logged_in(self, 'Error: You must be logged in to vote')
    ...
    def populate_list(self):
        self.candidate_list.delete(0, END)
        [self.candidate_list.insert(END, row) for row in db.get_candidate_by_position(self.combo_position.get())]
    ...
    def add_vote(self):
        ...
        if Validation.duplicate_rank_number([option_1, option_2, option_3, option_4]):
